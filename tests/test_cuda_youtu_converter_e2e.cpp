...
bool run(const char* package, Device device, InferenceResult& result) {
    ...
    if (architecture == metadata.end() || architecture->second != "mla" ||
        quantization == metadata.end() ||
        quantization->second != "w4g32") {
        return false;
    }
    ...
    if (engine.past_len() != 0 ||
        !copy_finite(
            engine.prefill_hidden({1, 2, 3, 4}), reset_prefill) ||
    ...
    if (device == Device::CUDA) {
        ...
        if (!stats.tracked || stats.native_calls == 0 ||
            ...
            std::fprintf(
                stderr,
                "Youtu CUDA operator coverage is native=%llu fallback=%llu\n",
...
int main(int argc, char** argv) {
    ...
    if (!run(argv[1], Device::CPU, cpu)) {
        std::fprintf(stderr, "tiny converted Youtu CPU inference failed\n");
        return 1;
    }
    ...
    if (!run(argv[1], Device::CUDA, cuda)) {
        std::fprintf(stderr, "tiny converted Youtu CUDA inference failed\n");
        return 1;
    }
    ...
    if (!valid)
        return 1;
    std::printf("Youtu BF16 checkpoint W4G32 CPU/CUDA E2E passed\n");
    return 0;
}
