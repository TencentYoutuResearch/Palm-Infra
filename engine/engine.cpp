...
bool LLMEngine::load_graph(Graph& g, ExecContext& exec_ctx, const char* path) {
    ...
    for (auto& node : g.nodes) {
        ...
        auto setup_quant_metadata = [&](const void* scales, uint32_t group_size,
                                        uint32_t num_groups, size_t scales_size,
            ...
            bool is_quantized =
                (t.prec == Precision::INT8 || t.prec == Precision::INT4);
            if (!is_quantized)
                return true;
            int64_t N = t.shape[0];
            int64_t K = t.shape[1];
            ...
            uint32_t groups_per_row =
                (uint32_t)((K + group_size - 1) / group_size);
            uint64_t expected_groups = (uint64_t)N * groups_per_row;
            bool int4_q4dot_layout = t.prec == Precision::INT4 &&
                                     (flags & MappedFile::FLAG_INT4_Q4DOT);
            bool int4_bg128_layout = t.prec == Precision::INT4 &&
            ...
            if (num_groups != expected_groups ||
                ...
                fprintf(stderr,
                        "Engine: quantized weight %s bad metadata (N=%lld "
                        "K=%lld group=%u groups=%u expected=%llu scales=%zu "
                        "data=%zu expected_data=%llu)\n",
                        label, (long long)N, (long long)K, group_size,
                        num_groups, (unsigned long long)expected_groups,
        ...
        if (package_weights_base_ != nullptr) {
            ...
            if (pit != package_weight_map_.end()) {
                ...
                if (!setup_quant_metadata(
                        scales, group_size, num_groups, (size_t)scales_size,
                        (size_t)data_size, flags, wref.c_str())) {
                    return false;
                }
        ...
        if (it != weight_map_.end()) {
            ...
            setup_weight(const_cast<void*>(shared_weights_[it->second].data()),
                         static_cast<Precision>(
                             shared_weights_[it->second].header().precision));
        } else {
            // Load new mmap
            MappedFile mf;
            ...
            setup_weight(const_cast<void*>(shared_weights_[idx].data()),
                         static_cast<Precision>(
                             shared_weights_[idx].header().precision));
            const auto& mapped = shared_weights_[idx];
            if (!setup_quant_metadata(mapped.scales(), mapped.group_size(),
                                      mapped.num_groups(), mapped.scales_size(),
                                      mapped.data_size(), mapped.header().flags,
                                      wpath.c_str())) {
                return false;
            }
        }
