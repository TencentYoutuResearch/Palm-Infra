...
`warmup=1` 且稠密权重锁定时的数据：

| SSD-backed expert 的 RAM cache | Decode | Expert-cache 命中率 | SSD 读取量 |
|---:|---:|---:|---:|
...
16 GiB 是该机器当前的甜点位：足以保存足够多的 expert、避免多数 SSD I/O，
又不会挤占稠密权重的工作集。相同 16 GiB 配置在严格的
`pp256 + tg64`、`warmup=3` 协议下，五个独立进程的中位数为 37.99 pp / 6.39 tg；
它的 decode 起始上下文比上面的交互式测量长得多（256 token）。

当前的 expert cache 策略有意保持简单。后续会改进 expert-cache 策略，并尝试在
MoE 层实际需要 expert 前、于 attention 阶段预测和预取路由 expert。
...
| Qwen3.5-122B-A10B W4（SSD offload） | **37.99** / **11.79** † | OOM | 可运行于 48GB Mac |

† Prefill 为标准 `pp256 + tg64`、`warmup=3` 的五进程中位数。Decode 为最佳真实
ChatML prompt 结果（16 个 prompt token、生成 128 个 token、`warmup=1`）。两者均使用
16 GiB 的 SSD-backed expert RAM cache 和锁定的稠密 mmap 权重。llama.cpp 的 CPU 基线无法装入 48GB RAM。
