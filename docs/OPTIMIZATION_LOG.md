...
**根因分析**：
- Apple M5 Pro FP16 FMA latency = 2 cycles, throughput = 2/cycle
...
**决策理由**：尝试 19 的 CONCAT/TILE bulk memcpy 在 `mollm_chat` 实际推理中
...
**验证**：
- `ctest` 14/14 通过
- `mollm_chat "你是谁"` 输出正常中文回复（"我是由OpenAI开发的AI助手..."），
...
实际推理中的 layout 不匹配 bug。`mollm_chat` 端到端文本验证是必要的。
...
做 8-way K-unroll + 4 累加器链，而 mollm 尝试 15 只用了 2-way。理论上 Apple M5 Pro FP16 FMA
...
**测量协议**：`mollm_bench --prompt-tokens 256 --max-new-tokens 64 --threads 4 --warmup 3`
...
**决策理由**：`mollm_chat` REPL 模式下，用户在输入提示前和读输出时，worker
...
2. 根因推测：park 期间 worker 释放 cache，下次 resume 时 L1/L2 更干净，
   减少了与主线程的 cache 竞争
3. 懒启动（`ensure_workers_started`）让 `mollm_chat` 启动后到第一次输入
   期间也无 CPU 占用
4. 实测 `mollm_chat "给我讲一个故事"` (25 token prompt, 128 token gen)：
...
**决策理由**：分析 ggml tinyBLAS 的调度方式发现，mollm GEMM 用 1D 静态 M-shard，
...
所有 ctest 15/15 通过，mollm_chat 输出正常。
...
**问题**：Qwen3.5 linear_attention 路径输出乱码。根因是 `_build_linear_attn_layer` 把 Gated Delta Rule 拆成 ~25 个细粒度 op（reshape/permute/slice/add/softplus/exp/mul/sigmoid/gdn/rms_norm/silu），而 mollm 的 reshape/permute/slice 是 zero-copy 元数据操作，对 matmul 输出（shape `[N,seq]` 但数据 `[seq,N]` row-major）的布局推理全部错位。
...
- `qwen35.py:348` `g.reshape(query, (head_dim, num_heads*seq_len))` 对 slice view materialize 时，mollm 的 reshape 按旧 shape 逻辑顺序复制，产出数据布局是 `[num_heads, head_dim, seq]` 而非期望的 `[head_dim, num_heads, seq]`。导致 RMSNorm 归一化维度错位。
- 这是 mollm reshape 对非 contiguous view materialize 的根本问题：不保证元素顺序与新 shape 一致。
